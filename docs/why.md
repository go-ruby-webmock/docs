...
`go-ruby-webmock/webmock` reimplements Ruby's `webmock` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-webmock/webmock` directly, with no Ruby runtime;
...
Because `webmock` is pure Go with cgo disabled, it also compiles to **WebAssembly**
